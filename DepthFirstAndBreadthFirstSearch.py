...
class Digraph(object):
    """
    edges is a deict mapping each
    node to a list of it's children
    """
    def __init__(self):
        self.edges = {}
...
def testBFS(source, destination):
    g = buildCityGraph(Digraph)
    sp = shortestPathBFS(g, g.getNode(source), g.getNode(destination), toPrint = False)
    if sp != None:
        print("Shortest path from", source, "to", destination, "is", printPath(sp))
    else:
        print("There is no path from", source, "to", destination)
